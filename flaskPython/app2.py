...
def connect():
	...
	return conn;
def selectData(select_value, lng, lon):
	conn = connect();
	cursor = conn.cursor('cursor_unique_name', cursor_factory=psycopg2.extras.DictCursor)
	cursor.execute("SELECT * FROM metadata_table WHERE tokens @@ to_tsquery('"+select_value +"') and ST_Contains(ST_GeometryFromText('POINT("+lng+"," + lan + ")', 4326));");
	return cursor;
...
def result():
	if request.method == 'POST':
		...
		if all_words[len(all_words)-1]=="|":
			all_words = all_words[:-1];
		mymetadata = selectData(all_words,json.loads(geocoding_result.text)['geonames'][0].lng,json.loads(geocoding_result.text)['geonames'][0].lat);
		my_metadata = [];
		i=0;
		for metadata in mymetadata.fetchall():
