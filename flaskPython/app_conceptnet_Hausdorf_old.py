...
def selectData(themes,location):
	...
	if(len(location) and len(themes)):
		...
		cursor.execute("SELECT mt.id,mt.id_increment,concat(mt.title,'-',ts_rank(mt.tokens,tsq),'-',ST_HausdorffDistance(st_geomfromgeojson('"+geocoding_result+"')::geometry, poly_geometry)), mt.description,ts_rank(mt.tokens,tsq) rank,ST_HausdorffDistance(st_geomfromgeojson('"+geocoding_result+"')::geometry, poly_geometry) area FROM metadata_table mt,to_tsquery('"+words +"') as tsq where ST_overlaps(st_geomfromgeojson('"+geocoding_result+"')::geometry, poly_geometry) and ts_rank(mt.tokens,tsq) > 0 order by rank desc,area asc;");
	elif(len(themes)):
		...
		geocoding_result = getGeom(location[0]);
		cursor.execute("SELECT mt.id,mt.id_increment,concat(mt.title,'-',ST_HausdorffDistance(st_geomfromgeojson('"+geocoding_result+"')::geometry, poly_geometry)), mt.description,ST_HausdorffDistance(st_geomfromgeojson('"+geocoding_result+"')::geometry, poly_geometry) area FROM metadata_table mt where ST_overlaps(st_geomfromgeojson('"+geocoding_result+"')::geometry, poly_geometry) order by area asc;");	
	return cursor;
Articles = Articles()
...
def conceptnetExpansion(theme):
	...
	# print('here');
	counter = 0;
	print(len(conceptnet_result1['edges']));
