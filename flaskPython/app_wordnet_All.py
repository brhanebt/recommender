...
def selectSimilar(id_increment):
	...
	try:
		cursor.execute("SELECT ts_rank(i.tokens, replace(strip(original.tokens)::text, ' ', '|')::tsquery) as similarity, i.id, i.id_increment, i.title, i.description, i.poly_geography,i.poly_geography FROM metadata_table i, (SELECT tokens, id_increment FROM metadata_table mt WHERE id_increment="+ str(id_increment) +" limit 1) AS original WHERE i.id_increment != original.id_increment ORDER BY similarity desc limit 5");
	except Exception:
		traceback.print_exc();
		return;
	return cursor;
...
def selectDetails(id_increment):
	conn = connect();
	cursor = conn.cursor('cursor_unique_name', cursor_factory=psycopg2.extras.DictCursor);
	cursor.execute("SELECT id_increment,id,title,description,st_asgeojson(poly_geography),st_asgeojson(poly_geography) FROM metadata_table mt where id_increment = "+id_increment+";");
	return cursor;	
